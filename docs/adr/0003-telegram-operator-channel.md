...
Superseded in part by ADR 0004 — 2026-08-09
...
- Convert authorized free text and `/ask` into a low-risk `operator.query` task. Execute it through the local Codex worker in read-only mode, with network disabled and explicit instructions forbidding tools, file inspection, credentials, and side effects. ADR 0004 replaces the free-text behavior while retaining `/ask`.
...
- Bot replies to completed work were initially pull-based through `/result`; ADR 0004 introduces the required durable outbound outbox.
