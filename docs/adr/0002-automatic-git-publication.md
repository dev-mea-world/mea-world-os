...
- each task operates in a deterministic, worker-owned Git worktree and local branch;
- Codex cannot use the network and never receives Git publishing credentials or responsibility;
- the worker independently checks the diff, blocks sensitive files and likely secrets, runs the repository test suite and typecheck, and creates the commit;
...
The main checkout, index and known-good branch remain untouched by loop updates. A network interruption after a successful push can be reconciled without duplicating the commit. Test, secret-policy and non-fast-forward failures leave the remote unchanged and are visible as explicit task/publication failures.

Automatic publication requires the worker configuration switch and usable Git authentication on the host. It does not grant permission to merge, force-push, delete branches, publish secrets, or bypass tests.
